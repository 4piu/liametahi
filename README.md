...
using a `jev` model and `task:` routing. The config file holds literal
credentials: it must be owned by you, and `config check` warns if its
permissions are group/world-readable.
