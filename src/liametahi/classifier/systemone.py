"""System One (`provider: systemone`) adapter, targeting the
`POST /v1/systemone` wire protocol -- pinned against TypeSafe AI's Jev,
the reference implementation, but not tied to that one vendor: any
service speaking the same request/response shape at its own `base_url`
works.

A system-one model is not a chat model: a decision model reached through
...
processor's `type`/`instructions`/`criteria` map onto a `Question`
...
**Request shape** (confirmed against a live call to Jev at
...
that index for `.value` (the service's own `legend` is not used -- it is
built from the same `criteria` list we submitted, in the same order, so
...
`None`: a system-one model has no separate confidence concept for a
`noul` answer, and `config.py` rejects a `processor: "name.confidence
..."` condition against any `noul` processor at load time for exactly
this reason.
...
`mails_per_request` must be `1` for `provider: systemone` (enforced at
config load, `config.ModelConfig._validate_provider_requirements`): it
answers one candidate per HTTP call, not a batch of candidates. A
processor answer that fails to parse out of an otherwise well-formed
multi-question response (missing key, wrong JSON type) is simply absent
from that candidate's `answers` map for this round (per
...
**Retry policy is a deliberate deviation from `openai.py`'s "transport
errors only, never a rejected response" policy**: this is a
single-purpose decision API behind a rate limiter, not a chat completion
request whose rejection typically means the request itself was
...
loop shape `openai.py` uses for transport errors. Every other 4xx/5xx
(`401` unauthorized, `422` malformed request, ...) fails immediately with
no retry: retrying a `401`/`422` can never succeed and only wastes the
retry budget that would otherwise be available for a genuine `429`/`529`.
"""
...
#: Status codes worth retrying (a deliberate deviation from `openai.py`'s
#: transport-only retry policy, see module docstring): temporary capacity
#: problems, not bad requests.
_RETRYABLE_STATUS_CODES = frozenset({429, 529})


class TransportError(Exception):
    """A systemone HTTP call failed after exhausting `max_retries` (for a
...
class SystemOneClassifier:
    """`Classifier` implementation for `provider: systemone`."""

    def __init__(self, config: ModelConfig, *, client: httpx.Client | None = None):
        if config.provider != "systemone":
            raise ValueError(
                f"SystemOneClassifier requires provider='systemone', "
                f"got {config.provider!r}"
            )
        if not config.base_url:
            raise ValueError("systemone requires 'base_url'")
        if config.mails_per_request != 1:
            raise ValueError("systemone requires mails_per_request == 1")
        self._config = config
    ...
    def classify(
        ...
        for candidate in candidates:
            ...
            try:
                ...
                logger.debug(
                    "systemone classify: candidate=%s failed: %s",
    ...
    def _call_one(
        ...
        if not isinstance(data, dict) or not isinstance(data.get("answers"), dict):
            raise TransportError(
                f"malformed systemone response (expected an 'answers' map): "
    ...
    def _post_with_retry(self, body: dict[str, object]) -> httpx.Response:
        ...
        (module docstring's deliberate deviation from `openai.py`'s
        transport-errors-only policy). A transport-level exception
        (connection reset, timeout) is also retried, matching every other
        adapter's behaviour for those. Every other status code --
        including every other 4xx/5xx -- fails immediately, no retry."""
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                last_exc = exc
                logger.debug(
                    "systemone: transport attempt %d/%d failed: %s",
                    attempt,
                    attempts,
                    exc,
                )
            ...
            logger.debug(
                "systemone: retryable HTTP %d on attempt %d/%d",
...
def _question_json(processor: OfferedProcessor) -> dict[str, object]:
    """Compile one offered processor into the protocol's `Question` shape
    -- a near-direct passthrough, since `OfferedProcessor`'s own fields
    already match its field names and per-type shapes exactly."""
...
def _parse_answer(
    ...
    if processor.type == "noul":
        ...
        # docstring: a system-one model has no separate confidence
        # concept for it.
